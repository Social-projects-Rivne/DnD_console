...
json UserActions::fLogin(std::string username, std::string password)
{
	json        user_login_data;  // json login result
	SHA256      sha256;			  // sha256 obj
	password = sha256(password);       // hash user pass
	user_login_data["action"] = "login";
	user_login_data["username"] = username; // insert into json key Login with value of user login input
	user_login_data["password"] = password;  // insert into json key Password with value of user hash pass
	return user_login_data;
}

json UserActions::fRegistration(std::string username, std::string email, std::string password, std::string repeat_password)
{
   ...
   //
	...
	while (password != repeat_password)     // check user passwords
	{
		//std::cout << "Password does not match! Retype your password: \n";
		//std::getline(std::cin, user_password);
		while (password.length() < 6)       // check for password length
		{
			std::cout << "Password length less than 6 symbols! Enter password: \n";
			//std::getline(std::cin, user_password);
		}
	...
	user_registration_data["email"] = email;
	user_registration_data["username"] = username;
